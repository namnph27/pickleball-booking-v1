...
async function createNewAdmin() {
  ...
  try {
    ...
    const usernameCheck = await client.query('SELECT * FROM admins WHERE username = $1', [newAdmin.username]);
    if (usernameCheck.rows.length > 0) {
      console.error(`Admin with username "${newAdmin.username}" already exists.`);
      return;
    }
    
    // Check if email already exists
    const emailCheck = await client.query('SELECT * FROM admins WHERE email = $1', [newAdmin.email]);
    if (emailCheck.rows.length > 0) {
      console.error(`Admin with email "${newAdmin.email}" already exists.`);
      return;
    }
    ...
    `, [newAdmin.username, hashedPassword, newAdmin.email, newAdmin.fullName, newAdmin.isSuperAdmin]);
